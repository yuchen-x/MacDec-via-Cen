...
def train(env_name, grid_dim, obs_one_hot, target_flick_prob, agent_trans_noise, target_rand_move, 
          ...
          sort_traj, mlp_layer_size, rnn, rnn_input_dim, rnn_layer_num, rnn_h_size, gru, 
    ...
    if env_name.startswith('OSD'):
        ...
        env = ENV(1,2,tuple(grid_dim),
                  terminate_step=env_terminate_step,
                  obs_one_hot=obs_one_hot, 
                  target_flick_prob=target_flick_prob, 
                  agent_trans_noise=agent_trans_noise, 
                  tgt_random_move=target_rand_move)
    ...
    while team.episode_count < total_epi:
        ...
        if not step % target_update_freq: 
            ...
            if (time.time()-t) / 3600 >= 23 and save_ckpt:
...
def main():
    ...
    parser.add_argument('--mlp_layer_size',     action='store',         type=int,            default=32,            help='MLP layer dimension of decentralized policy-net')
    parser.add_argument('--rnn_layer_num',      action='store',         type=int,            default=1,             help='Number of RNN layers')
    parser.add_argument('--rnn_h_size',         action='store',         type=int,            default=64,            help='RNN hidden layer dimension of decentralized policy-net')
