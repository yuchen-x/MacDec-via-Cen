...
from rlmamr.MA_dec_cen_hddrqn_sep.team import Team_RNN
from rlmamr.MA_dec_cen_hddrqn_sep.utils.memory import ReplayMemory_rand, ReplayMemory_epi
from rlmamr.MA_dec_cen_hddrqn_sep.utils.utils import save_check_point
from rlmamr.MA_dec_cen_hddrqn_sep.learning_methods import QLearn_squ_dec_cen_0
...
def train(env_name, grid_dim, obs_one_hot, target_flick_prob, agent_trans_noise, target_rand_move, 
          h_speed_ps, obs_h_wait, tb_m_speed, tb_m_noisy, f_p_obj_tc, f_l_obj_tc, f_m_noisy, d_pen,
    ...
    if env_name.startswith('OSD'):
        env_cen = ENV(terminate_step=env_terminate_step, human_speed_per_step=[h_speed_ps], TB_move_speed=tb_m_speed, TB_move_noisy=tb_m_noisy, fetch_pass_obj_tc=f_p_obj_tc, fetch_look_for_obj_tc=f_l_obj_tc, fetch_manip_noisy=f_m_noisy, delay_delivery_penalty=d_pen)
        env_dec = ENV(terminate_step=env_terminate_step, human_speed_per_step=[h_speed_ps], TB_move_speed=tb_m_speed, TB_move_noisy=tb_m_noisy, fetch_pass_obj_tc=f_p_obj_tc, fetch_look_for_obj_tc=f_l_obj_tc, fetch_manip_noisy=f_m_noisy, delay_delivery_penalty=d_pen)
    else:
        env = ENV(tuple(grid_dim),terminate_step=env_terminate_step)

    # create replay buffer
    if sample_epi:
        memory_cen = ReplayMemory_epi(env_cen.n_agent, env_cen.obs_size, batch_size, size=replay_buffer_size)
        memory_dec = ReplayMemory_epi(env_dec.n_agent, env_dec.obs_size, batch_size, size=replay_buffer_size)
    else:
        memory_cen = ReplayMemory_rand(env_cen.n_agent, env_cen.obs_size, trace_len, batch_size, size=replay_buffer_size)
        memory_dec = ReplayMemory_rand(env_dec.n_agent, env_dec.obs_size, trace_len, batch_size, size=replay_buffer_size)
    ...
    # create team
    team = Team_RNN((env_cen, env_dec), n_env, (memory_cen, memory_dec), env_dec.n_agent, QLearn_squ_dec_cen[l_mode], h_stable_at,
    ...
    while team.episode_count < total_epi:
        ...
        if not step % target_update_freq: 
            ...
            if (time.time() - t_ckpt) // 3600 >= 5 and save_ckpt:
...
def main():
    ...
    parser.add_argument('--obs_h_wait', action='store_true')
    ...
    parser.add_argument('--f_m_noisy', action='store', type=float, default=0.0)
    parser.add_argument('--d_pen', action='store_true') # delay delivery penalty
    ...
    parser.add_argument('--cen_explore', action='store_true')
    parser.add_argument('--cen_explore_end', action='store', type=int, default=100*1000)
    parser.add_argument('--explore_switch', action='store_true')
    parser.add_argument('--db_step', action='store_true')

    parser.add_argument('--l_mode', action='store', type=int, default=3)
    ...
    parser.add_argument('--dec_mlp_layer_size', action='store', type=int, default=32)
    parser.add_argument('--cen_mlp_layer_size', action='store', type=int, default=32)
    parser.add_argument('--rnn_layer_num', action='store', type=int, default=1)
    parser.add_argument('--dec_rnn_h_size', action='store', type=int, default=64)
    parser.add_argument('--cen_rnn_h_size', action='store', type=int, default=128)

    parser.add_argument('--n_run', action='store', type=int, default=0)
