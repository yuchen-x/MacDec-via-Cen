...
def train(env_name, grid_dim, obs_one_hot, target_flick_prob, agent_trans_noise, target_rand_move, 
          h_speed_ps, tb_m_speed, tb_m_noisy, f_p_obj_tc, f_l_obj_tc, f_m_noisy, d_pen,
          ...
          sort_traj, dec_mlp_layer_size, cen_mlp_layer_size, rnn, rnn_input_dim, rnn_layer_num, dec_rnn_h_size, cen_rnn_h_size, 
          eval_freq, run_id, resume, save_ckpt, seed, save_dir, device, **kwargs):
    ...
    if env_name.startswith('OSD'):
        env_cen = ENV(terminate_step=env_terminate_step, 
        ...
        env_dec = ENV(terminate_step=env_terminate_step, 
                      human_speed_per_step=[h_speed_ps], 
                      TB_move_speed=tb_m_speed, 
                      TB_move_noisy=tb_m_noisy, 
                      fetch_pass_obj_tc=f_p_obj_tc, 
        ...
        env_cen = ENV(tuple(grid_dim),terminate_step=env_terminate_step)
        env_dec = ENV(tuple(grid_dim),terminate_step=env_terminate_step)
