    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                child.send((last_obs, dec_last_h, cen_last_h, last_id_action, last_id_valid))
            elif cmd == 'close':
                child.close()
                break
            else:
...
class EnvsRunner(object):
    ...
    def step(self):
        ...
        for idx, parent in enumerate(self.parents):
            # get next action
            if self.h_explore:
                actions, self.dec_h_states[idx], self.cen_h_states[idx] = self.get_actions(self.last_obses[idx], self.dec_h_states[idx], self.cen_h_states[idx], self.actions[idx], self.last_valids[idx])
            else:
                actions, self.dec_hidden_states[idx], self.cen_hidden_states[idx] = self.get_actions(self.last_obses[idx], self.dec_h_states[idx], self.cen_h_states[idx], self.actions[idx], self.last_valids[idx])
        ...
        for idx, parent in enumerate(self.parents):
            ...
            if env_return[-3]:
                ...
                self.last_obses[idx], self.dec_h_states[idx], self.cen_h_states[idx], self.actions[idx], self.last_valids[idx] = parent.recv()
    ...
    def reset(self):
        ...
        self.last_obses, self.dec_h_states, self.cen_h_states, self.actions, self.last_valids = [list(i) for i in zip(*[parent.recv() for parent in self.parents])]
