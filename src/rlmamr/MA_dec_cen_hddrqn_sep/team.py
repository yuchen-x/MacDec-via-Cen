...
class Team:
    ...
    def __init__(self, envs, memories, n_agent, h_stable_at, dynamic_h=False, hysteretic=None, discount=0.99,
...
class Team_RNN(Team):
    ...
    def __init__(self, envs, n_env, memories, n_agent, training_method, h_stable_at, discount=0.99, sample_epi=False, 
        ...
        self.envs_runner_dec = EnvsRunner(self.env_dec, self.memory_dec, n_env, h_explore, self.get_next_action_dec)
    ...
    def get_next_action_cen(self, joint_obs, dec_h_states, cen_h_state, cen_last_action, last_valid, eval=False):
        ...
        with torch.no_grad():
            if max(last_valid) == 1.0:
                ...
                a = get_conditional_argmax(Q, get_conditional_action(torch.cat(cen_last_action).view(1,-1), torch.cat(last_valid).view(1,-1)), self.env_cen.n_action).item()
    ...
    def get_next_action_dec(self, joint_obs, dec_h_states, cen_h_state, cen_last_action, last_valid, eval=False):
                ...
                if last_valid[agent.idx]:
                    ...
                    if not eval:
                        if self.soft_action_selection:
                            ...
                            if np.random.random() < self.epsilon:
                                a = self.env_dec.action_space_sample(i)
    ...
    def train(self):
        ...
        self.training_method(self.env_dec, self.agents, self.cen_controller, (cen_batch, dec_batch_cen, dec_batch_dec), self.hysteretic, self.discount, **self.hyper_params)
    ...
    def evaluate(self, n_episode=1):
        ...
        for _ in range(n_episode):
            ...
            while not t:
                a, dec_h_states, cen_h_state = self.get_next_action_dec(last_obs, dec_h_states, cen_h_state, cen_last_action, last_valid, eval=True)
                a, obs, r, t, v = self.env_dec.step(a)
